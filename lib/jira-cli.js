(function() {
  ...
  Logger = (function() {
    ...
    Logger.prototype.log = function(text, color) {
      ...
      return console.log(color(text, color));
    };
  ...
  JiraHelper = (function() {
    ...
    JiraHelper.prototype.addIssue = function(summary, description, issueType, project) {
      ...
      this.jira.addNewIssue(newIssue, function(error, response) {
        if (response != null) {
          if (response != null) {
            _this.response = response;
          }
          return _this.log.log(("Issue " + response.key + " has ") + ("been " + (color("created", "green"))));
        } else {
          if (error != null) {
            _this.error = error;
          }
          return _this.log.error("Error creating issue: " + (JSON.stringify(error)));
        }
      });
      return this.dieWithFire();
    };
